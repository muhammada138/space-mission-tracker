...
class StarshipTestsView(APIView):
    ...
    _COMPILED_KEYWORDS_REGEX = re.compile('|'.join(re.escape(k) for k in KEYWORDS))
    _COMPILED_NEGATIVE_REGEX = re.compile('|'.join(re.escape(k) for k in NEGATIVE_KEYWORDS))
    ...
    def get(self, request):
        ...
        for d in checklist_defs:
            d['regex'] = re.compile('|'.join(re.escape(k) for k in d['keywords']))

        # Pre-compile fast path regex for all keywords
        all_keywords_regex = re.compile('|'.join(re.escape(k) for d in checklist_defs for k in d['keywords']))
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                for entry in root.findall('atom:entry', ns):
                    ...
                    if self._COMPILED_KEYWORDS_REGEX.search(title_lower):
                        if self._COMPILED_NEGATIVE_REGEX.search(title_lower):
                            continue
