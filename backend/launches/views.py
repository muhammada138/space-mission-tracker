...
from .services import (
    ...
    get_past_launches,
    get_launch_by_api_id,
    refresh_launches_by_api_ids,
    get_active_launches,
    _to_dt,
    _FAR_PAST,
    _FAR_FUTURE,
...
def _filter_and_deduplicate(launches, source, seen=None):
    ...
    seen_names: set[str] = (
        set()
    )  # Secondary dedup — prevents same mission showing twice
...
class UpcomingLaunchesView(APIView):
    ...
    def get(self, request):
        ...
        now = timezone.now()
        final_launches = [
            l for l in final_launches if _to_dt(l.get("launch_date"), _FAR_PAST) >= now
        ]
...
class PastLaunchesView(APIView):
    ...
    def get(self, request):
        ...
        final_launches = [
            l for l in final_launches if _to_dt(l.get("launch_date"), _FAR_FUTURE) < now
        ]
        ...
        final_launches.sort(
            key=lambda l: _to_dt(l.get("launch_date"), _FAR_PAST), reverse=True
        )
        ...
        try:
            upcoming_names = set(
                Launch.objects.filter(launch_date__gt=now).values_list(
                    "name", flat=True
                )
            )
            ...
            final_data = [
                l
                for l in final_data
                if l.get("name") not in upcoming_names
                or _to_dt(l.get("launch_date"), _FAR_PAST) > suppress_cutoff
            ]
        except Exception:
            pass
    ...
    def get(self, request):
        ...
        try:
            for h in history_data:
                ...
                if (
                    ldate < now
                    and ldate >= two_years_ago
                    and "Success" in (h.get("status") or "")
                ):
                    name = h.get("name", "")
        ...
        try:
            ...
            def sort_key(x):
                d = (
                    getattr(x, "launch_date", None)
                    if hasattr(x, "launch_date")
                    else x.get("launch_date")
                )
            ...
            if final_data:
                self._cache = {"data": final_data, "expires": now + timedelta(hours=24)}
            return Response(final_data)
...
class LaunchDetailView(APIView):
    ...
    def get(self, request, api_id):
        ...
        try:
            ...
            is_active_window = ldate != _FAR_PAST and (
                now - timedelta(hours=6)
            ) <= ldate <= (now + timedelta(minutes=15))
            ...
            is_recent_nonterminal = (
                ldate != _FAR_PAST
                and ldate < now
                and (now - ldate) < timedelta(days=7)
                and "success" not in (launch.status or "").lower()
                and "fail" not in (launch.status or "").lower()
            )
        ...
        return Response(
            {"detail": "Launch not found."}, status=status.HTTP_404_NOT_FOUND
        )
...
class ISSCrewView(APIView):
    ...
    def _get_wiki_summary_sync(client, name):
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if (
                        ...
                        or "can refer to:" in extract.lower()
                        or data.get("type") == "disambiguation"
                    ):
                        return None
                    return {
                        "wiki_thumbnail": data.get("originalimage", {}).get(
                            "source", ""
                        )
        ...
        if not wiki_data or len(wiki_data.get("wiki_extract", "")) < 50:
            wiki_data = (
                _fetch(f"{name} (astronaut)")
                or _fetch(f"{name} (cosmonaut)")
                or wiki_data
            )
    ...
    def get(self, request):
        ...
        with httpx.Client(timeout=15) as client:
            ...
            try:
                ...
                for a in results:
                    ...
                    entry = {
                        ...
                        or a.get("profile_image_thumbnail")
                        or "",
                    ...
                    if ss and isinstance(ss, dict):
                        ...
                        try:
                            ...
                            if (
                                "shenzhou" in last_flight.lower()
                                or "tiangong" in last_flight.lower()
                            ):
                                craft = "Tiangong"
                            else:
            ...
            if not crew:
                ...
                return Response(
                    {"error": "Crew data temporarily unavailable"}, status=503
                )
                        ...
                        for future in as_completed(futures):
                            try:
                                ...
                                if wiki:
                                    ...
                                    if wiki.get("wiki_extract") and len(
                                        wiki["wiki_extract"]
                                    ) > len(entry.get("bio", "")):
    ...
    def get(self, request, api_id):
        ...
        try:
            if api_key:
                ...
                description = (
                    raw.get("weather", [{}])[0].get("description", "clear").title()
                )
            ...
            rules = [
                {
                    "name": "Wind Speed",
                    "value": f"{wind_knots:.1f} kts",
                    "go": wind_knots < 30,
                    "limit": "< 30 kts",
                },
                {
                    "name": "Visibility",
                    "value": f"{visibility_mi:.1f} mi",
                    "go": visibility_mi >= 5,
                    "limit": "≥ 5 mi",
                },
                {
                    "name": "Thunderstorm",
                    "value": "Clear" if not thunderstorm else "Active",
                    "go": not thunderstorm,
                    "limit": "None within 10 mi",
                },
                {
                    "name": "Temperature",
                    "value": f"{temp_c:.0f}°C",
            ...
            overall = (
                "GO"
                if go_count == len(rules)
                else ("HOLD" if go_count < len(rules) - 1 else "MARGINAL")
            )
...
class StarshipTestsView(APIView):
    ...
    def get(self, request):
        ...
        checklist_defs = [
            {
                "key": "s39_static",
                "label": "Ship 39 Static Fire (Massey's)",
                "keywords": ["ship 39 static", "s39 static"],
            },
            {
                "key": "b19_static",
                "label": "Booster 19 Static Fire",
                "keywords": ["booster 19 static", "b19 static"],
            },
            {
                "key": "stacking",
                "label": "B19 / S39 Stacking (Pad 2)",
                "keywords": ["stacking", "stacked", "full stack"],
            },
            {
                "key": "wdr",
                "label": "Flight 12 Wet Dress Rehearsal",
                "keywords": ["wdr", "wet dress"],
            },
            {
                "key": "faa",
                "label": "FAA Flight 12 Launch License",
        ...
        try:
            ...
            try:
                ...
                if tx_resp.status_code == 200:
                    ...
                    for tweet in tweets:
                        ...
                        t_lower_decoded = t_lower.encode("utf-8").decode(
            ...
            resp = httpx.get(
                rss_url, timeout=12, headers=headers, follow_redirects=True
            )
            ...
            if not entries:
                logger.info(
                    f"RSS failed or empty for {self.CHANNEL_ID}, attempting scraper fallback..."
                )
                scrape_url = f"https://www.youtube.com/channel/{self.CHANNEL_ID}/videos"
                sresp = httpx.get(
                    scrape_url, headers=headers, timeout=10, follow_redirects=True
                )
            ...
            for d in checklist_defs:
                dynamic_checklist.append(
                    {"task": d["label"], "status": task_status[d["key"]]}
                )

            return Response({"videos": entries[:12], "checklist": dynamic_checklist})
        except Exception as e:
            logger.error(f"Critical error in StarshipTestsView: {e}")
            return Response({"videos": [], "checklist": fallback_checklist}, status=200)
