...
def mock_httpx_get_responses(kp_value=0, num_flares=0, noaa_fail=False, nasa_fail=False):
    def side_effect(url, **kwargs):
        if 'noaa.gov' in url:
            ...
            response = MagicMock()
            response.status_code = 200
            response.json.return_value = [{'flrID': f'flr{i}'} for i in range(num_flares)]
            return response
        return MagicMock()
    return side_effect
...
class TestSpaceWeatherView:
    ...
    def test_caching(self, api_client):
        ...
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=1, num_flares=1)) as mock_get:
    ...
    def test_nominal_level_quiet(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=1, num_flares=1)):
    ...
    def test_nominal_level_unsettled(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=2, num_flares=1)):
            response = api_client.get(url)
            assert response.status_code == 200
            assert response.data['level'] == 'nominal'
    ...
    def test_moderate_level_by_kp(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=4, num_flares=1)):
            response = api_client.get(url)
            assert response.status_code == 200
    ...
    def test_moderate_level_by_flares(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=1, num_flares=3)):
            response = api_client.get(url)
            assert response.status_code == 200
            assert response.data['level'] == 'moderate'
            assert response.data['label'] == 'Moderate Activity'
    ...
    def test_severe_level_by_kp(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=7, num_flares=1)):
            response = api_client.get(url)
            assert response.status_code == 200
    ...
    def test_severe_level_by_flares(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=1, num_flares=6)):
            response = api_client.get(url)
            assert response.status_code == 200
            assert response.data['level'] == 'severe'
            assert response.data['label'] == 'Storm Active'
    ...
    def test_noaa_failure(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(num_flares=1, noaa_fail=True)):
            response = api_client.get(url)
            assert response.status_code == 200
    ...
    def test_nasa_failure(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(kp_value=1, nasa_fail=True)):
            response = api_client.get(url)
            assert response.status_code == 200
            assert response.data['kp'] == 1
    ...
    def test_both_failures(self, api_client):
        url = reverse('space-weather')
        with patch('httpx.get', side_effect=mock_httpx_get_responses(noaa_fail=True, nasa_fail=True)):
            response = api_client.get(url)
            assert response.status_code == 200
            assert response.data['kp'] == 0
            assert response.data['flares'] == 0
    ...
    def test_overall_exception(self, api_client):
        url = reverse('space-weather')
        with patch('os.environ.get', side_effect=Exception("Environment variable error")):
            response = api_client.get(url)
            assert response.status_code == 200
            assert response.data['level'] == 'nominal'
